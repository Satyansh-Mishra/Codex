  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://codex-sk6m.onrender.com/api/auth/login",
        {
          method: "POST",
      ...
      <div className="w-full lg:w-[45%] h-[30vh] lg:h-screen relative overflow-hidden bg-[#1A1A1A]">
        ...
        >
          <div className="flex items-center gap-4 mb-2">
            ...
            <span className="text-white/80 font-mono text-[10px] uppercase tracking-widest">
              System Active
            </span>
          </div>
          <p className="text-white font-serif text-2xl">
            4,281 ingredients <br />
            <span className="italic text-[#A8B545]">identified today.</span>
          </p>
        </motion.div>

        <Link
          href="/"
          className="absolute top-8 left-8 text-white flex items-center gap-4 hover:gap-6 transition-all group z-10"
        >
          <ArrowLeft
            size={18}
            className="transition-transform group-hover:-translate-x-1"
          />
          <span className="text-xs uppercase tracking-[0.2em] font-medium mt-1">
            Return
          </span>
        </Link>
      </div>
        ...
        <div className="w-full max-w-lg mx-auto lg:mx-0">
          ...
          <div className="mb-16">
            <RevealText delay={0.1}>
              <h1 className="text-6xl lg:text-7xl font-serif tracking-tighter">
                Welcome
              </h1>
            </RevealText>
            <RevealText delay={0.2}>
              <h1 className="text-6xl lg:text-7xl font-serif tracking-tighter italic text-[#A8B545]">
                back.
              </h1>
            </RevealText>
          </div>
          ...
          >
            <p className="text-xs font-mono uppercase tracking-widest text-[#1A1A1A]/50">
              ...
              <Link
                href="/signup"
                className="text-[#1A1A1A] font-bold ml-2 hover:text-[#A8B545] transition-colors relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[1px] after:bg-current after:scale-x-100 hover:after:scale-x-0 after:transition-transform after:origin-right"
              >
                Create Account
              </Link>
            </p>
