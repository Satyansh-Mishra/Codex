  ...
  return (
    <div className="w-full flex gap-2 h-1 mt-[-10px] mb-8">
      {[1, 2, 3].map((level) => (
        <div
          key={level}
          className="flex-1 h-full bg-black/5 rounded-full overflow-hidden"
        >
          <motion.div
            initial={false}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://codex-sk6m.onrender.com/api/auth/signup",
        {
          method: "POST",
    ...
    <main className="min-h-screen flex flex-col lg:flex-row bg-[#FDFCF6] text-[#1A1A1A] selection:bg-[#A8B545] selection:text-white">
      ...
      <div className="w-full lg:w-[45%] h-[30vh] lg:h-screen relative overflow-hidden bg-[#1A1A1A] order-1 lg:order-2">
        ...
        >
          <ChefHat
            size={32}
            className="text-[#A8B545] ml-auto mb-4 opacity-80"
          />
      ...
      <div className="w-full lg:w-[55%] h-[70vh] lg:h-screen flex flex-col justify-center px-8 sm:px-16 lg:px-24 xl:px-32 relative z-10 order-2 lg:order-1">
        <Link
          href="/"
          className="absolute top-8 left-8 text-[#1A1A1A] flex items-center gap-4 hover:gap-6 transition-all group z-10"
        >
          <ArrowLeft
            size={18}
            className="transition-transform group-hover:-translate-x-1"
          />
          <span className="text-xs font-mono uppercase tracking-[0.2em] font-medium mt-1">
            Return
          </span>
        </Link>
        ...
        <div className="w-full max-w-lg mx-auto lg:mx-0 lg:ml-auto">
          ...
          <div className="mb-12">
            ...
            <RevealText delay={0.2}>
              <h1 className="text-6xl lg:text-7xl font-serif tracking-tighter italic text-[#D48C70]">
                movement.
              </h1>
            </RevealText>
          </div>
          ...
          >
            <p className="text-xs font-mono uppercase tracking-widest text-[#1A1A1A]/50">
              ...
              <Link
                href="/login"
                className="text-[#1A1A1A] font-bold ml-2 hover:text-[#D48C70] transition-colors relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[1px] after:bg-current after:scale-x-100 hover:after:scale-x-0 after:transition-transform after:origin-right"
              >
                Log In
              </Link>
            </p>
